...
func TestVersionsGetViewerWidth(t *testing.T) {
	t.Run("fullscreen returns width minus padding", func(t *testing.T) {
		s := NewVersionsScreenWithData("plan.md", busquets.MarkdownThemeASCII, nil, 100, 40, false, false)
...
func TestVersionsUpdateListItems(t *testing.T) {
	t.Run("empty versions produces empty list", func(t *testing.T) {
		s := NewVersionsScreenWithData("plan.md", busquets.MarkdownThemeASCII, nil, 80, 24, false, false)
	...
	t.Run("item count matches versions count", func(t *testing.T) {
		...
		s := NewVersionsScreenWithData("plan.md", busquets.MarkdownThemeASCII, versions, 80, 24, false, false)
...
func TestVersionCopyKeyEmitsCopyMsg(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			s := NewVersionsScreenWithData("plan.md", busquets.MarkdownThemeASCII, versions, 100, 40, false, false)
			s.focus = tc.focus
