...
func (q *Queries) DeletePlan(ctx context.Context, arg DeletePlanParams) error {
	_, err := q.db.Exec(ctx, deletePlan, arg.FileName, arg.SyncSource)
	return err
}
...
func (q *Queries) DeletePlanVersions(ctx context.Context, planID int64) error {
	_, err := q.db.Exec(ctx, deletePlanVersions, planID)
	return err
}
...
SELECT id, file_name, sync_source, file_path, title, content, created_at, modified_at, indexed_at, file_size, word_count FROM plans WHERE file_name = $1 AND sync_source = $2 LIMIT 1
`
...
func (q *Queries) GetPlanByFileNameAndSource(ctx context.Context, arg GetPlanByFileNameAndSourceParams) (Plan, error) {
	row := q.db.QueryRow(ctx, getPlanByFileNameAndSource, arg.FileName, arg.SyncSource)
	var i Plan
	err := row.Scan(
		&i.ID,
		&i.FileName,
...
func (q *Queries) GetPlanCommentCounts(ctx context.Context) ([]GetPlanCommentCountsRow, error) {
	rows, err := q.db.Query(ctx, getPlanCommentCounts)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT id, plan_id, content, created_at, updated_at FROM plan_comments WHERE plan_id = $1 ORDER BY created_at ASC
`

func (q *Queries) GetPlanComments(ctx context.Context, planID int64) ([]PlanComment, error) {
	rows, err := q.db.Query(ctx, getPlanComments, planID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
VALUES ($1, $2, $3, $4) RETURNING id, plan_id, content, created_at, updated_at
`
...
func (q *Queries) SetConnectorRole(ctx context.Context, arg SetConnectorRoleParams) error {
	_, err := q.db.Exec(ctx, setConnectorRole, arg.ConnectorName, arg.Role)
	return err
}
