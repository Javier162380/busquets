...
SELECT id, file_name, sync_source, file_path, title, content, created_at, modified_at, indexed_at, file_size, word_count FROM plans WHERE id = ? LIMIT 1
`
...
func (q *Queries) InsertPlan(ctx context.Context, arg InsertPlanParams) (int64, error) {
...
func (q *Queries) ListAllPlansFull(ctx context.Context) ([]ListAllPlansFullRow, error) {
	rows, err := q.db.QueryContext(ctx, listAllPlansFull)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT id, plan_id, version_number, file_path, content, word_count, created_at FROM plan_versions WHERE plan_id = ?
`

func (q *Queries) ListPlanVersionsAll(ctx context.Context, planID int64) ([]PlanVersion, error) {
	rows, err := q.db.QueryContext(ctx, listPlanVersionsAll, planID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) UpdatePlanFilePath(ctx context.Context, arg UpdatePlanFilePathParams) error {
	_, err := q.db.ExecContext(ctx, updatePlanFilePath, arg.FilePath, arg.ID)
	return err
}
...
func (q *Queries) UpdatePlanVersionFilePath(ctx context.Context, arg UpdatePlanVersionFilePathParams) error {
	_, err := q.db.ExecContext(ctx, updatePlanVersionFilePath, arg.FilePath, arg.ID)
	return err
}
